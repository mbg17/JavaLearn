...
public class Interval2D {
    ...
    public boolean intersects(Interval2D that) {
        if (!this.x.intersects(that.x) || !this.y.intersects(that.y)) {
            return false;
        }
        return true;
    }
    ...
    public static void main(String[] args) {
        double xmin = Double.parseDouble("15");
        double xmax = Double.parseDouble("23");
        double ymin = Double.parseDouble("15");
        double ymax = Double.parseDouble("34");
        int trials = Integer.parseInt("65");
